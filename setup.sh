...
#cd $HOME
#cd termuxstyling
echo Script made by:- Expert Anonymous
#Uninstall
if [ -e ".user.cfg" ]
then
...
updatedw() {
	...
	then
		...
		if [[ \$var == *\"up to date\"* ]];
		then 
			clear && echo \"Already running the latest version!!\" && echo ------------------------------------- && figlet \$(sed '1q;d' 'ver.cfg') && figlet Expert
		else
			git pull
			bash setup.sh;
		...
		git clone https://github.com/icammaci/termuxstyling
		cd termuxstyling
		prm sh
		clear
		bash
...
mdv README.md
cd $pdir
echo Subscribe to our YT channel MR CRACKED
echo twitter.com/MrCracked3
